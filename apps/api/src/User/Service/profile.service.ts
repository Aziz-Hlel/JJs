...
class ProfileService {
  async getUserPage(queryParams: UserPageQuery): Promise<Page<UserProfileRowResponse>> {
    ...
    if (cachedResult) {
      logger.debug('[Cache hit:ProfileService] Returning cached user page');
      return cachedResult;
    }
    logger.debug('[Cache miss:ProfileService] Fetching user page');
    ...
    await cacheService.set({ key: cachedKey, value: userPage, ttlSeconds: 60 });
