/*
  Warnings:

  - A unique constraint covering the columns `[referenceCode]` on the table `User` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `referenceCode` to the `User` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "User" ADD COLUMN     "referenceCode" VARCHAR(6) NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "User_referenceCode_key" ON "User"("referenceCode");
